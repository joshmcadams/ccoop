...
The command reads `coop_config.json`, creates a clean scene in a **separate background process**, runs the geometry/installation checks, and saves only after those pass. It refuses to run in an interactive Blender session. The command above writes a candidate so the working model remains available during review. `--render` additionally creates `generated/assembly-preview.png` and `generated/roof-installation.png`.
...
- selected completed-phase holds through frame 680.

Hardware envelopes, terrain/footing intersections, structural capacity, subframe motion, every moving timber assembly, tolerances and supplier-specific connector clearance are outside this validation. See the pending design tasks in DESIGN.md.
...
For an assembly-stage preview, open the candidate, select the required frame from BUILD.md and render with its supplied camera. Restore frame 680 for the current roof-sheet view. `--render` refreshes the final assembly and roof-installation previews; supplementary images such as `floor-framing.png` and `floor-installation.png` must be refreshed separately after relevant changes. Reports and previews describe the most recent run, so do not present old results as evidence for a new candidate.

## Files

- `coop.blend`: regenerated working model; frame 680 shows the skeleton and roof covering placement.
...
The camera uses Blender Workbench studio lighting, so separate light objects are unnecessary. Use the timeline markers to inspect assembly phases. Wall frames, nests, cladding and doors are pending.

Roof sheets are representative corrugated meshes with candidate bonded fleece. Roof screws, flashings and final condensation/ventilation details remain pending. Frame 465 retains the uncovered framing view; frame 600 shows sheet 3 arriving. The roof checks do not validate supplier tooling, fleece compression, fixing capacity or weathertightness.
